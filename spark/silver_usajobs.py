...
def main(run_date: date) -> None:
    ...
    base = raw.select(
        F.concat(F.lit("usajobs-"), col_or_null(raw, "MatchedObjectId").cast("string")).alias(
            "posting_id"
        ),
    ...
    matched = F.array_distinct(
        F.array_compact(
            F.array(
                *[
                    F.when(
                        F.lower(F.coalesce(F.col("summary"), F.lit(""))).contains(s),
                        F.lit(s),
                    )
    ...
    print(
        f"DONE. USAJobs jobs: {jobs_silver.count()} | skills: {skills_silver.count()}",
        flush=True,
    )
    spark.stop()
